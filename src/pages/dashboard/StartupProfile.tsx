...
import { Startup, useStartupStore } from '../../store/startupStore';
...
const StartupProfile = () => {
  ...
  const renderMilestones = (milestones: Startup["milestones"]) => {
    if(!milestones) return null;

    // For each category
    return Object.entries(milestones).map(([category, stages]) => {
      // for each stage in the category
      const stageWithTrue = Object.entries(stages || {}).map(([stage, checklist]) => {
        ...
        return (
          <div key={stage} className="ml-4">
            <div className="font-medium text-gray-700">{stage}</div>
            <div className="flex flex-wrap gap-1 mt-1">
              {trueMilestones.map(([milestone, _]) => (
                <span key={milestone} className="inline-flex items-center px-2 py-0.5 rounded bg-green-100 text-green-800 text-xs font-semibold border border-green-200">
      ...
      if (stageWithTrue.length === 0) return null;

      return (
        <div key={category} className="flex flex-col mr-4">
          <div className="text-xs font-semibold text-primary-700 uppercase tracking-wide mb-1">{category}</div>
          {stageWithTrue}
        </div>
      );
    });
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:ml-6 flex-grow">
            ...
            <div className="mt-4 flex flex-col sm:flex-row sm:items-center text-sm text-gray-600 gap-y-2 sm:gap-x-4">
              ...
              {/**Replacing the Building2 stage div with the updated milestone */}
              {/*
              <div className="flex items-center">
            ...
            <div className="flex items-start flex-wrap gap-x-4">
                <Building2 size={16} className="mt-0.5 mr-1 text-gray-400"/>
                <div className="flex flex-wrap gap-x-4">
                  {renderMilestones(startup.milestones)}
                </div>
            </div>
            */}
          </div>
